...
static int load_png_image(const char *filename, uint8_t **out_image, uint32_t *width, uint32_t *height);
static void modeset_draw(const char *image_path);
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <card0> <png_file_path>\n", argv[0]);
        return 1;
    }
	...
	printf("start draw PNG\n");
	modeset_draw(image_path);
	sleep(100);
	/* cleanup everything */
	modeset_cleanup(fd);
...
// Helper function to load a PNG image and convert it to RGB888 format
static int load_png_image(const char *filename, uint8_t **out_image, uint32_t *width, uint32_t *height) {
    ...
    *height = png_get_image_height(png, info);
    ...
    // Convert any color type to 8-bit RGB
    ...
    *out_image = malloc(3 * (*width) * (*height)); // RGB888
    ...
    for (int y = 0; y < *height; y++) {
        for (int x = 0; x < *width; x++) {
            png_bytep px = &(row_pointers[y][x * 4]); // Assuming PNG with RGBA
            (*out_image)[(y * (*width) + x) * 3] = px[0];     // R
            (*out_image)[(y * (*width) + x) * 3 + 1] = px[1]; // G
            (*out_image)[(y * (*width) + x) * 3 + 2] = px[2]; // B
        }
    }
...
static void modeset_draw(const char *image_path) {
    ...
    if (load_png_image(image_path, &image, &img_width, &img_height) < 0) {
        fprintf(stderr, "Failed to load image %s\n", image_path);
        return;
    }

    // Iterate over each device and draw the image
    for (iter = modeset_list; iter; iter = iter->next) {
        uint32_t x, y;
        uint8_t *map = iter->map;

        for (y = 0; y < iter->height; ++y) {
            for (x = 0; x < iter->width; ++x) {
                if (x < img_width && y < img_height) {
                    // Draw the image pixel (RGB888 format)
                    uint8_t *pixel = &image[(y * img_width + x) * 3];
                    map[(y * iter->stride) + (x * 4)] = pixel[2];     // B
                    map[(y * iter->stride) + (x * 4) + 1] = pixel[1]; // G
                    map[(y * iter->stride) + (x * 4) + 2] = pixel[0]; // R
                } else {
                    // Fill the remaining area with black
                    map[(y * iter->stride) + (x * 4)] = 0;
                    map[(y * iter->stride) + (x * 4) + 1] = 0;
                    map[(y * iter->stride) + (x * 4) + 2] = 0;
                }
            }
        }
    }
